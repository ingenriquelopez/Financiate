...
const PlanDeAhorro = () => {
  ...
  // Estado para controlar la visibilidad de RegistrarAhorro y el ID del plan
  const [showRegistrarAhorro, setShowRegistrarAhorro] = useState(false);
  const [planId, setPlanId] = useState(null); // Estado para almacenar el ID del plan seleccionado

  // Función que se ejecuta al hacer clic en el botón de registrar
  const handleRegistrarAhorro = (planId) => {
    setShowRegistrarAhorro(true); // Muestra el componente RegistrarAhorro
    setPlanId(planId); // Establece el ID del plan seleccionado
  };
  ...
  const handleOpenModal = (plan = null) => {
    ...
    if (plan) {
      setFormData({
        ...
        monto_acumulado: plan.monto_acumulado || "",
      });
      ...
      <table className="table table-striped table-bordered table-hover">
        <thead>
          <tr>
            ...
            <th className="text-center">Fecha Objetivo</th>
            <th className="text-center">Monto objetivo</th>
            <th className="text-center">Monto acumulado</th>
                ...
                <tr key={plan.id}>
                  ...
                  <td>
                    <button
                      ...
                      style={{ color: 'green' }}
                      onClick={() => handleRegistrarAhorro(plan.id)}
                    >
                       🐷
                    </button>
                    {/* Renderiza el componente RegistrarAhorro solo si showRegistrarAhorro es true */}
                    {showRegistrarAhorro && (
                      <RegistrarAhorro
                        planId={planId} // Pasamos el ID del plan seleccionado al componente
                        onClose={() => setShowRegistrarAhorro(false)} // Función para cerrar el modal
                      />
                    )}
                    <button
